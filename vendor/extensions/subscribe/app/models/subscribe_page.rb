class SubscribePage < Page
  ...
  tag "subscribe:form" do |tag|
    a = tag.attr.dup
    list = a.delete('list')
    attrs = defaulted_attrs(a, 'action' => tag.globals.page.url, 'method' => 'post')
    %{<form#{attrs}>#{tag.expand}</form>}
  end
  
  tag "subscribe:form:first_name_input" do |tag|
    attrs = defaulted_attrs(tag.attr, 'name' => 'first_name', 'value' => first_name)
    %{<input#{attrs} />}
  end
  
  tag "subscribe:form:last_name_input" do |tag|
    attrs = defaulted_attrs(tag.attr, 'name' => 'last_name', 'value' => last_name)
    %{<input#{attrs} />}
  end
  
  tag "subscribe:form:email_input" do |tag|
    attrs = defaulted_attrs(tag.attr, 'name' => 'email', 'value' => email)
    %{<input#{attrs} />}
  end
  
  tag "subscribe:form:action_select" do |tag|
    attrs = defaulted_attrs(tag.attr, 'name' => 'ml_action')
    options = ['Subscribe', 'Unsubscribe'].map do |opt|
      selected = %{ selected="true"} if opt.downcase == ml_action
  ...
  tag "subscribe:form:list_select" do |tag|
    attrs = defaulted_attrs(tag.attr, 'name' => 'list')
    lists = List.find_all
  ...
  def render
    if request.post?
      ...
      @ml_action     = get_param(:ml_action).downcase
      if @ml_action =~ /^(subscribe|unsubscribe)$/
        if @list
  ...
  private
    ...
    def defaulted_attrs(tag_attrs, defaults)
